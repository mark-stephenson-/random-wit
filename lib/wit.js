...
function Wit(opts) {
  ...
  const continueRunActions = (sessionId, currentRequest, message, prevContext, i) => {
    return (json) => {
      ...
      if (json.type === 'msg' && prevContext.rand_reply) {
        const response = {
          text: json.msg,
          quickreplies: json.quickreplies,
        };
        if(prevContext.rand_reply) {
          this._rand_msg.push(response);
        }
      ...
      } else if (json.type === 'action' && json.action === 'send_rand_reply') {

        request.text = this._rand_msg[Math.floor(Math.random() * this._rand_msg.length)].text;
